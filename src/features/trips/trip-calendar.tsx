...
const TYPE_LIGHT: Record<string, string> = {
  ...
  ROAD_TRIP:   'bg-blue-50 text-blue-700 border-blue-200',
  WORKATION:   'bg-violet-50 text-violet-700 border-violet-200',
  LEISURE:     'bg-pink-50 text-pink-700 border-pink-200',
            ...
            {Array.from({ length: totalCells }).map((_, i) => {
              ...
              return (
                <div
                  ...
                  className={`min-h-[90px] p-1.5 border-b border-r border-gray-100 cursor-pointer transition-colors group
                    ${isSelected ? 'bg-orange-50' : 'hover:bg-gray-50'}`}
                >
                  <div className="flex justify-end mb-1">
                    <span className={`text-xs font-semibold w-6 h-6 flex items-center justify-center rounded-full
                      ${isToday ? 'bg-orange-500 text-white' : isSelected ? 'text-orange-600' : 'text-gray-500 group-hover:text-gray-900'}`}>
                      {dayNum}
                    </span>
                  </div>
                      ...
                      return (
                        <div
                          ...
                          className={`text-[10px] font-medium px-1.5 py-0.5 rounded-sm truncate
                            ${isStart
                              ? `${TYPE_COLORS[trip.tripType] ?? 'bg-gray-400'} text-white`
        ...
        <div className="mt-4 flex flex-wrap gap-3 px-1">
          {Object.entries(TRIP_TYPE_LABELS).map(([key, label]) => (
            <div key={key} className="flex items-center gap-1.5">
              <span className={`h-2.5 w-2.5 rounded-sm ${TYPE_COLORS[key] ?? 'bg-gray-400'}`} />
        ...
        <div className="sticky top-24">
          {selectedDate ? (
                ...
                <div className="space-y-3">
                  {selectedTrips.map((trip) => (
                    <Link key={trip.id} href={`/trips/${trip.id}`}>
                      <article className="bg-white rounded-2xl border border-gray-100 overflow-hidden hover:shadow-md hover:border-gray-200 transition-all group">
                        {trip.coverImage ? (
                        ...
                        <div className="p-3">
                          <div className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold border mb-2 ${TYPE_LIGHT[trip.tripType] ?? 'bg-gray-50 text-gray-600 border-gray-200'}`}>
                            {TRIP_TYPE_LABELS[trip.tripType as keyof typeof TRIP_TYPE_LABELS]}
                          </div>
                          <h4 className="text-sm font-semibold text-gray-900 line-clamp-1 group-hover:text-orange-600 transition-colors">
                            {trip.title}
                          </h4>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 p-8 text-center">
              ...
              <p className="text-sm font-medium text-gray-500 mb-1">Select a date</p>
              <p className="text-xs text-gray-400">Click any day to see trips happening then</p>
            </div>
          )}
        </div>
      </div>
