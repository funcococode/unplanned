    ...
    <div className="bg-white border border-gray-100 rounded-2xl overflow-hidden">
      ...
      <div className="px-4 pt-4 pb-3 border-b border-gray-50">
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-xs font-semibold text-gray-900">My Packing List</span>
          <span className="ml-auto text-[10px] text-gray-400 italic">only you see this</span>
        </div>
        ...
        {total > 0 && (
          <div className="space-y-1">
            <div className="flex justify-between items-center">
              <span className="text-[10px] text-gray-400">{checked} of {total} packed</span>
          ...
          <>
            {items.length === 0 && !adding ? (
              ...
              <div className="text-center py-5">
                <div className="w-10 h-10 bg-violet-50 rounded-2xl flex items-center justify-center mx-auto mb-2">
                  <ListChecks className="h-5 w-5 text-violet-300" />
                </div>
                <p className="text-xs text-gray-400 mb-3">Nothing here yet.</p>
                {essentialItems.length > 0 && (
                  <button
                    onClick={handleImport}
                    disabled={importing}
                    className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-violet-600 bg-violet-50 hover:bg-violet-100 rounded-lg transition-colors disabled:opacity-60"
                  >
              ...
              <ul className="space-y-0.5 mb-2">
                {items.map((item) => (
                  <li
                    key={item.id}
                    className="flex items-center gap-2 rounded-lg px-1 py-1.5 group hover:bg-gray-50 transition-colors"
                  >
                    {/* Checkbox */}
                    <button
                    ...
                    {/* Text */}
                    <span className={`text-xs flex-1 leading-snug ${item.checked ? 'line-through text-gray-300' : 'text-gray-700'}`}>
                      {item.text}
                    </span>
            ...
            {adding ? (
              <div className="flex gap-1.5 mt-1">
                <input
                  ...
                  className="flex-1 px-2.5 py-1.5 text-xs border border-violet-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-violet-400 min-w-0"
                  autoFocus
              ...
              <div className="flex items-center justify-between mt-1">
                ...
                {essentialItems.length > 0 && items.length > 0 && (
                  <button
                    onClick={handleImport}
                    disabled={importing}
                    className="flex items-center gap-1 text-[10px] text-gray-400 hover:text-violet-500 transition-colors disabled:opacity-60"
                  >
                    {importing ? <Loader2 className="h-3 w-3 animate-spin" /> : <Download className="h-3 w-3" />}
                    Import
                  </button>
